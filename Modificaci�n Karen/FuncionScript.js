    ...
    for (i=0; i<(args.length-2); i+=3) { test=args[i+2]; val=document.getElementById(args[i]);
	  ...
	  if(document.getElementById("CorreoElectronico").value != ""){   
	     ...
	     if(corvalpri == false){
		     ...
		     document.getElementById("cajaCuen").value = "";
	     }
	  }
    if (errors) alert('El(los) siguiente(s) error(es) ha(n) ocurrido: '+errors);
    else Registro(); 
...
function Registro()
{
   document.frmregistro.action = "insertarClientes.php";
